...
## 7. Orchestration Visualized (Mermaid)

```mermaid
graph TD
    Start([User Topic]) --> Planner[Planner Agent]
    Planner --> ResA[Researcher A (Grok)]
    Planner --> ResB[Researcher B (Qwen)]
    ResA --> Combined[Combined Research]
    ResB --> Combined
    Combined --> LoopStart{Drafting Loop x3}
    
    subgraph Iterative Drafting
        LoopStart --> Writer[Writer Agent]
        Writer --> Draft[Draft]
        Draft --> Critic[Critic Agent]
        Critic --> Scores{Score & Feedback}
        Scores -->|Next Iteration| Writer
    end
    
    Scores -->|Select Best| BestDraft[Best Draft]
    BestDraft --> Finalizer[Finalizer Agent]
    Finalizer --> FinalText[Final Post]
    FinalText --> VisPrompt[Visualizer (Prompt)]
    VisPrompt --> ImageGen[Qwen Image Gen]
    ImageGen --> FinalImage[Final Image]
    FinalText --> End([Final Output])
    FinalImage --> End
```
