...
import { AgentType, QualityCritiqueInput, QualityCritique, WorkflowState, Improvement } from '../types';
...
export class QualityCriticAgent extends AgentBase<QualityCritiqueInput, QualityCritique> {
  ...
  private async evaluateQuality(input: QualityCritiqueInput): Promise<QualityCritique> {
    ...
    const schema = {
      ...
      finalRecommendation: { type: 'string', enum: ['accept', 'improve', 'reject'] },
  ...
  private buildEvaluationPrompt(input: QualityCritiqueInput): string {
    ...
    prompt += `
      ...
      3. Accuracy (0-100): Factual correctness based on provided analysis
      4. Overall Quality (0-100): Weighted assessment of all factors
      Provide specific improvements with severity levels (low/medium/high) and actionable suggestions.
