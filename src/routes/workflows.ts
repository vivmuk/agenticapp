...
import { WorkflowStartInput, HumanReviewInput, ReviewPriority, HumanReviewStatus, TriggerReason, ReviewComment } from '../types';
...
router.post('/start', async (req, res) => {
  ...
  try {
    ...
    if (!topic || typeof topic !== 'string' || topic.trim().length === 0) {
      logger.warn('Validation failed: topic missing');
    ...
    if (maxCycles && (typeof maxCycles !== 'number' || maxCycles < 1 || maxCycles > 10)) {
      logger.warn('Validation failed: maxCycles invalid', { maxCycles });
      return res.status(400).json({
        success: false,
        error: 'maxCycles must be a number between 1 and 10',
      });
    }

    if (qualityThreshold && (typeof qualityThreshold !== 'number' || qualityThreshold < 0 || qualityThreshold > 1)) {
      logger.warn('Validation failed: qualityThreshold invalid', { qualityThreshold });
      return res.status(400).json({
        success: false,
        error: 'qualityThreshold must be a number between 0 and 1',
      });
    }
...
router.get('/', async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: 'Internal server error while listing workflows',
    });
  }
});
...
router.get('/:id', async (req, res) => {
  try {
    ...
    logger.info('Getting workflow status', { workflowId: id });

    const workflowState = await workflowManager.getWorkflowState(id);
    if (!workflowState) {
    ...
    res.json({
      ...
      data: {
        ...
        currentContent: workflowState.currentContent,
        currentAccuracyCritique: workflowState.currentAccuracyCritique,
        currentQualityCritique: workflowState.currentQualityCritique,
    ...
    res.status(500).json({
      success: false,
      error: 'Internal server error while retrieving workflow',
    });
  }
});

// POST /api/workflows/:id/human-review - Submit human feedback
router.post('/:id/human-review', async (req, res) => {
  try {
    const { id } = req.params;
    const { action, feedback, customEdits }: HumanReviewInput = req.body;
    ...
    if (!action || !['accept', 'improve', 'reject'].includes(action)) {
      return res.status(400).json({
        success: false,
        error: 'Action must be one of: accept, improve, reject',
      });
    }
    ...
    if (customEdits && typeof customEdits !== 'object') {
      return res.status(400).json({
        success: false,
        error: 'Custom edits must be an object',
      });
    }

    logger.info('Submitting human review', { workflowId: id, action, feedback });

    const workflowState = await workflowManager.submitHumanReview(id, {
      action,
      feedback,
      customEdits,
    });
    ...
    res.status(500).json({
      success: false,
      error: 'Internal server error while submitting human review',
    });
  }
});

// GET /api/workflows/:id/versions - Get content version history
router.get('/:id/versions', async (req, res) => {
  try {
    ...
    logger.info('Getting workflow versions', { workflowId: id });
    ...
    if (versions.length === 0) {
      return res.status(404).json({
        success: false,
        error: 'Workflow not found or no versions available',
      });
    }
    ...
    logger.error('Failed to get workflow versions', {
      workflowId: req.params.id,
      error: (error as Error).message,
    });
    res.status(500).json({
      success: false,
      error: 'Internal server error while retrieving workflow versions',
    });
  }
});
