...
export const WorkflowControls: React.FC = () => {
  ...
  const handleReset = () => {
    if (currentWorkflow) {
      console.log('Reset workflow:', currentWorkflow.id);
    }
  };
  ...
  const getStatusText = (status: WorkflowStatus) => {
    switch (status) {
      ...
      case WorkflowStatus.RUNNING:
        return 'Running';
      case WorkflowStatus.CONTENT_GENERATION:
        return 'Generating Content';
      case WorkflowStatus.WEB_SEARCH_CRITIC:
        return 'Validating Accuracy';
      case WorkflowStatus.QUALITY_CRITIC:
        return 'Evaluating Quality';
      case WorkflowStatus.HUMAN_REVIEW:
        return 'Human Review';
      case WorkflowStatus.COMPLETED:
        return 'Completed';
      case WorkflowStatus.FAILED:
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center space-x-4">
          ...
          {currentWorkflow && (
            <div className="flex items-center space-x-2">
              <span className={`status-badge ${getStatusColor(currentWorkflow.status)}`}>
        ...
        {currentWorkflow && (
          <div className="flex items-center space-x-2 ml-auto">
            ...
            >
              ...
              <span>
                {currentWorkflow.status === WorkflowStatus.RUNNING ? 'Pause' : 'Start'}
              </span>
            </button>
            ...
            <button
              ...
              disabled={
                !currentWorkflow ||
                currentWorkflow.status === WorkflowStatus.INITIALIZING
              }
            >
          ...
          <div className="mt-4">
            ...
            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
              <motion.div
                ...
                initial={{ width: 0 }}
                animate={{
                  width: `${(currentWorkflow.currentCycle / currentWorkflow.maxCycles) * 100}%`
                }}
      ...
      {/* Quality Score */}
      {currentWorkflow && currentWorkflow.finalQualityScore && (
          ...
          <div className="flex justify-between items-center">
            ...
            <span
              className={`text-lg font-bold ${
                currentWorkflow.finalQualityScore >= 7
                  ? 'text-green-600'
                  : currentWorkflow.finalQualityScore >= 5
                  ? 'text-yellow-600'
                  : 'text-red-600'
              }`}
            >
              {currentWorkflow.finalQualityScore.toFixed(1)}/10
            </span>
          </div>
        </div>
      )}
      ...
      {/* Human Review Status */}
      {currentWorkflow && currentWorkflow.humanReviewRequired && (
          ...
          <div className="flex justify-between items-center">
            ...
            <span
              className={`text-sm font-medium ${
                currentWorkflow.humanReviewProvided ? 'text-green-600' : 'text-yellow-600'
              }`}
            >
              {currentWorkflow.humanReviewProvided ? 'Provided' : 'Pending'}
            </span>
          </div>
          {currentWorkflow.humanReviewFeedback && (
